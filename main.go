...
// convert EMU -> px (approx): 1 inch = 914400 EMU, 96 px/in
...
func buildSlideHTML(bgURI string, boxes []textfinder.TextBox, mathBoxes []mathfinder.MathBox) string {
	...
	for _, mb := range mathBoxes {
		...
		// Math equations without background - transparent
		sb.WriteString(fmt.Sprintf(
	...
	// render text boxes with improved styling
...
func main() {
	...
	bgURI := ""
	if mediaPath != "" {
		a := NormalizeMediaPath(mediaPath)
		uri, err := mediaDataURIFromZip(r, a)
		fmt.Println("Background media path in PPTX:", a, err)
		if err == nil {
			bgURI = uri
		}
	}
	...
	fmt.Println("Found", len(mathBoxes), "math boxes", mathBoxes)

	// write HTML
	html := buildSlideHTML(bgURI, boxes, mathBoxes)
	if err := os.WriteFile("slide.html", []byte(html), 0644); err != nil {
		fmt.Println("write slide.html:", err)
		return
	}
	fmt.Println("Wrote slide.html — open in browser.")
}
